...
csv::find_loops(const calculation_path &calculationPath, const csv::table_t &table) {
    ...
    // мапа, в которой отмечено какие ячейки уже были пройдены
    ...
    while (!std::isdigit(nextCell[0])) {
        ...
        if (!complexCell.secondArg.empty()) {
            ...
            //bool before = (visited.count(complexCell.firstArg) != 0);

            // если ячейка уже была пройдена, значит, мы нашли зависимость. Проверка для первого адреса.
            // для остановки рекурсии, если путь состоит из выражений (ARG1 OP ARG2).
            if (visited[complexCell.firstArg]) {
...
//            }
            ...
            // если ячейка уже была пройдена, значит, мы нашли зависимость. Проверка для второго адреса.
            // для остановки рекурсии, если путь состоит из выражений (ARG1 OP ARG2).
            if (visited[complexCell.secondArg]) {
        ...
        // если ячейка уже была пройдена, значит, мы нашли зависимость. Для случая, когда в ячейке один адрес.
